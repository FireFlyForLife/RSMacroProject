...
namespace RSMacroProgram
{
    internal class Hotkey_Control : IMessageFilter
	{
		...
		public Hotkey_Control() : this(Keys.None, false, false, false, false)
		{
			// No work done here!
		}
		
		public Hotkey_Control(Keys keyCode, bool shift, bool control, bool alt, bool windows)
		{
			// Assign properties
		...
		public bool Register(Control windowControl)
        {
			...
			this.id = Hotkey_Control.currentID;
			Hotkey_Control.currentID = Hotkey_Control.currentID + 1 % Hotkey_Control.maximumID;

			// Translate modifier keys into unmanaged version
			uint modifiers = (this.Alt ? Hotkey_Control.MOD_ALT : 0) | (this.Control ? Hotkey_Control.MOD_CONTROL : 0) |
							(this.Shift ? Hotkey_Control.MOD_SHIFT : 0) | (this.Windows ? Hotkey_Control.MOD_WIN : 0);

			// Register the hotkey
			if (Hotkey_Control.RegisterHotKey(windowControl.Handle, this.id, modifiers, keyCode) == 0)
			{ 
		...
		public void Unregister()
		{
			...
			if (!this.windowControl.IsDisposed)
			{
				...
				if (Hotkey_Control.UnregisterHotKey(this.windowControl.Handle, this.id) == 0)
				{ throw new Win32Exception(); }
			}
		...
		public bool PreFilterMessage(ref Message message)
		{
			...
			if (message.Msg != Hotkey_Control.WM_HOTKEY)
			{ return false; }
